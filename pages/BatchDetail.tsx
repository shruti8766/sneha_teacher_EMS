      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                  <p className="text-xs text-orange-700 uppercase font-medium flex items-center gap-2 mb-2">
                    <BookOpen size={16} /> Subject
                  </p>
                  <p className="text-2xl font-bold text-orange-900">{batch.subject}</p>
                </div>
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <p className="text-xs text-blue-700 uppercase font-medium mb-2">
                    {batch.board} • Standard {batch.standard}
                  </p>
                  <p className="text-lg font-semibold text-blue-900">Board & Level</p>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {students.map(student => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600">{student.board}</td>
                          <td className="px-6 py-4 text-sm text-gray-600">{student.standard}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                              student.active ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-700'
                            }`}>
          ...
          {activeTab === 'info' && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
              <div>
                <h3 className="text-sm font-semibold text-gray-600 uppercase mb-4">Batch Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Name</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{batch.name}</p>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Subject</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{batch.subject}</p>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Board</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{batch.board}</p>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Standard</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{batch.standard}</p>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Max Students</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{batch.maxStudents || '∞'}</p>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 uppercase font-medium">Currently Enrolled</label>
                    <p className="text-lg font-medium text-gray-900 mt-1">{students.length}</p>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl shadow-sm border border-orange-200 p-6 space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-orange-900 uppercase mb-4 flex items-center gap-2">
                <Award size={16} /> Summary
              </h3>
              <div className="space-y-4">
                <div className="bg-white bg-opacity-60 rounded-lg p-4">
                  <p className="text-xs text-orange-700 uppercase font-medium">Subject</p>
                  <p className="text-2xl font-bold text-orange-900 mt-1">{batch.subject}</p>
                </div>
                <div className="bg-white bg-opacity-60 rounded-lg p-4">
                  <p className="text-xs text-orange-700 uppercase font-medium">Students Enrolled</p>
                  <p className="text-2xl font-bold text-orange-900 mt-1">{students.length}</p>
                </div>
                <div className="bg-white bg-opacity-60 rounded-lg p-4">
                  <p className="text-xs text-orange-700 uppercase font-medium">Board & Standard</p>
            ...
            {batchTeacher && (
              <div className="border-t border-orange-200 pt-4">
                <h3 className="text-sm font-semibold text-orange-900 uppercase mb-3 flex items-center gap-2">
                ...
                <div className="bg-white bg-opacity-80 rounded-lg p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 truncate">{batchTeacher.name}</p>
                      <p className="text-xs text-gray-600 truncate">{batchTeacher.email}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                required
                type="text"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none bg-white text-gray-900"
                value={editForm.name}
