...
const SidebarLink = ({ to, icon: Icon, label, onClick, isCollapsed, isDarkMode }: any) => (
  <NavLink
    to={to}
    onClick={onClick}
    className={({ isActive }) =>
    ...
    <div className={`flex h-screen overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
          ...
          <SidebarLink to="/student/dashboard" icon={LayoutDashboard} label="Dashboard" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
          ...
          <SidebarLink to="/student/homework" icon={BookOpen} label="My Homework" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
          <SidebarLink to="/student/tests" icon={ClipboardList} label="Tests & Results" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
          <SidebarLink to="/student/attendance" icon={CalendarCheck} label="Attendance" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
          ...
          <SidebarLink to="/student/fees" icon={CreditCard} label="Fee Details" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
          <SidebarLink to="/student/messages" icon={MessageSquare} label="Messages" onClick={closeMenu} isCollapsed={isCollapsed} isDarkMode={isDarkMode} />
        ...
        <div className={`p-3 border-t transition-colors duration-300 ${isDarkMode ? 'border-gray-700 bg-gradient-to-t from-gray-700 to-gray-800' : 'border-gray-100 bg-gradient-to-t from-blue-50 to-white'}`}>
          {!isCollapsed && (
            ...
            >
              ...
              <p className={`text-xs capitalize font-medium transition-colors ${isDarkMode ? 'text-gray-400 group-hover:text-blue-300' : 'text-gray-500 group-hover:text-blue-600'}`}>Student</p>
            </button>
          )}
          <button
            onClick={logout}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto overflow-x-hidden">
        <header className={`border-b p-4 pr-6 flex items-center justify-between sticky top-0 z-40 gap-4 transition-colors duration-300 rounded-2xl m-2 mb-0 mr-2 overflow-visible ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className={`font-semibold capitalize md:hidden flex-1 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
            {location.pathname.replace('/student/', '').replace('detail', '').replace('Detail', '') || 'Dashboard'}
          </div>
